...
const FROM_EMAIL = "Audilix <onboarding@resend.dev>"; // email par défaut Resend
...
async function sendReportEmail(toEmail, companyName, report) {
  ...
  const risquesHtml = risques
    .map((r) => `<li style="margin-bottom:8px;">⚠️ ${r}</li>`)
    .join("");
  ...
  const html = `
  <div style="font-family:'DM Sans',Arial,sans-serif;max-width:620px;margin:auto;background:#0B2545;border-radius:16px;overflow:hidden;">
    ...
    <div style="padding:40px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.08);">
      ...
      <h1 style="margin:0;font-size:28px;font-weight:700;color:#ffffff;letter-spacing:-0.02em;">AUDILIX</h1>
    </div>
    ...
    <div style="padding:40px;text-align:center;">
      ...
      <div style="display:inline-block;width:110px;height:110px;border-radius:50%;border:3px solid ${scoreColor};line-height:110px;font-size:36px;font-weight:700;color:${scoreColor};background:rgba(255,255,255,0.03);">
        ${score}
      </div>
    ...
    <div style="padding:24px 40px;text-align:center;">
      <p style="font-size:11px;color:rgba(255,255,255,0.2);margin:0;letter-spacing:0.06em;">© 2026 AUDILIX — contact.audilix@gmail.com</p>
    </div>
  </div>
  `;
...
app.post("/webhook/tally", async (req, res) => {
  try {
    ...
    const dpo = getValue("responsable") || "Non précisé";
...
// ─── Health check ──────────────────────────────────────────────
app.get("/", (req, res) => {
  res.json({ status: "Audilix backend en ligne ✅", version: "2.1" });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () =>
  console.log("🚀 Audilix backend en ligne sur le port", PORT)
);
